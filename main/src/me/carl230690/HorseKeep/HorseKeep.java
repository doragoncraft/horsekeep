 ...
 public class HorseKeep extends JavaPlugin
   implements Listener
 {
   ...
   private void checkConfigurationSections()
   {
     if (!getConfig().isConfigurationSection("horses")) getConfig().createSection("horses");
   }
   ...
   public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
     if (cmd.getName().equalsIgnoreCase("horse")) {
       ...
       else if ((args[0].equalsIgnoreCase("list")) || (args[0].equalsIgnoreCase("l")))
       {
         ...
         List horsesList = this.khorse.getOwnedHorses((Player)sender);
 
         for (String horseId : horsesList)
         {
           String stored;
           String stored;
           if (this.khorse.isStored(UUID.fromString(horseId))) stored = ChatColor.RED + " [Stored]"; else {
             stored = "";
           }
       ...
       else if ((args[0].equalsIgnoreCase("setid")) || (args[0].equalsIgnoreCase("id")))
       {
         if (args.length < 2)
         {
           sender.sendMessage(this.prefix + ChatColor.GOLD + "Missing horse identifier");
           return true;
         }
         ...
         {
           sender.sendMessage(this.prefix + ChatColor.GOLD + "This horse doesn't exist");
           return true;
         }
 
         if ((!this.khorse.isHorseOwner(horseIdentifier, sender.getName())) && (!perm.has(sender, "horsekeep.admin")))
         {
           sender.sendMessage(this.prefix + ChatColor.GOLD + "You don't own this horse");
           return true;
         }
 
         if (args.length < 3)
         {
           sender.sendMessage(this.prefix + ChatColor.GOLD + "Missing horse new identifier");
          return true;
         }
 
         Boolean hasWhiteSpace = Boolean.valueOf(false);
         for (char c : args[2].toCharArray()) {
           if (Character.isWhitespace(c)) {
             hasWhiteSpace = Boolean.valueOf(true);
           }
         }
 
         if (hasWhiteSpace.booleanValue())
         {
           sender.sendMessage(this.prefix + ChatColor.GOLD + "Identifier may not contain whitespace");
           return true;
         }
 
         if (this.khorse.isHorseIdentifierTaken(args[2]))
         {
           sender.sendMessage(this.prefix + "Identifier has already been taken, please choose another.");
           return true;
         }
         ...
         sender.sendMessage(this.prefix + "New identifier set to " + args[2]);
       }
       else if (args[0].equalsIgnoreCase("tp"))
       {
         ...
         if (args.length < 2)
         {
           player.sendMessage(this.prefix + ChatColor.GOLD + "Missing horse identifier");
           return true;
         }
         ...
         if (!this.khorse.horseIdentifierExists(horseIdentifier))
         {
           player.sendMessage(this.prefix + ChatColor.GOLD + "This horse doesn't exist");
          return true;
         }
 
         if ((!this.khorse.isHorseOwner(horseIdentifier, player.getName())) && (!perm.has(player, "horsekeep.admin")))
         {
           player.sendMessage(this.prefix + ChatColor.GOLD + "You don't own this horse");
           return true;
         }
         ...
         if (response.equals(HorseTeleportResponse.NOT_TELEPORTED_ENTITY_DELETED)) {
           player.sendMessage(this.prefix + ChatColor.RED + "Horse " + horseIdentifier + " was not found (dead or deleted), removing from config...");
           this.khorse.removeHorse(horseIdentifier);
         }
         else if (response.equals(HorseTeleportResponse.NOT_TELEPORTED_WRONG_WORLD)) {
           player.sendMessage(this.prefix + ChatColor.GOLD + "Horse " + horseIdentifier + " was not teleported because located in another world");
         }
         else if (response.equals(HorseTeleportResponse.NOT_TELEPORTED)) {
           player.sendMessage(this.prefix + ChatColor.GOLD + "Horse " + horseIdentifier + " was not teleported because of unknown error");
         }
         else if (response.equals(HorseTeleportResponse.NOT_TELEPORTED_STORED)) {
           player.sendMessage(this.prefix + ChatColor.GOLD + "Horse " + horseIdentifier + " was not teleported because currently stored. Summon it by using /horse summon " + horseIdentifier);
         }
 
         player.sendMessage(this.prefix + "Done");
       }
       else if (args[0].equalsIgnoreCase("tpall"))
       {
         ...
         List horsesList = this.khorse.getOwnedHorses(player);
 
         if (horsesList.size() == 0)
         {
           player.sendMessage(this.prefix + ChatColor.GOLD + "You don't own horses");
           return true;
         }
       ...
       else if ((args[0].equalsIgnoreCase("members")) || (args[0].equalsIgnoreCase("m")))
       {
         ...
         List horsesList = this.khorse.getHorseMembers(this.khorse.getHorseUUID(horseIdentifier));
       ...
       else if ((args[0].equalsIgnoreCase("addmember")) || (args[0].equalsIgnoreCase("addm")))
       {
         ...
         if (args.length < 3)
         {
           sender.sendMessage(this.prefix + ChatColor.GOLD + "Missing player");
           return true;
         }
         ...
         {
           sender.sendMessage(this.prefix + ChatColor.GOLD + playerName + " have access to this horse already.");
           return true;
         }
 
         this.khorse.addHorseMember(horseIdentifier, playerName);
 
         sender.sendMessage(this.prefix + "Added new member " + playerName + " to horse " + horseIdentifier);
       }
       else if ((args[0].equalsIgnoreCase("delmember")) || (args[0].equalsIgnoreCase("delm")))
       {
         ...
         if (!this.khorse.isHorseMember(horseIdentifier, playerName))
         {
           sender.sendMessage(this.prefix + ChatColor.GOLD + "This player is not a member of this horse");
           return true;
         }
 
         this.khorse.removeHorseMember(horseIdentifier, playerName);
 
         sender.sendMessage(this.prefix + "Removed member " + playerName + " from horse " + horseIdentifier);
       }
        else if ((args[0].equalsIgnoreCase("unprotect")) || (args[0].equalsIgnoreCase("up")))
       {
         ...
         sender.sendMessage(this.prefix + "Horse is now un-protected");
       }
       else if (args[0].equalsIgnoreCase("store"))
       {
         ...
         {
           sender.sendMessage(this.prefix + ChatColor.GOLD + "You must be riding your protected horse to use this command");
           return true;
         }
         ...
         {
           sender.sendMessage(this.prefix + ChatColor.GOLD + "This horse is not protected");
           return true;
         }
         ...
         sender.sendMessage(this.prefix + "Horse is now stored. You can summon it by using /horse summon " + this.khorse.getHorseIdentifier(horse.getUniqueId()));
       }
       else if (args[0].equalsIgnoreCase("summon"))
       {
         ...
         {
           player.sendMessage(this.prefix + ChatColor.GOLD + "This horse is currently not stored");
           return true;
         }
 
         this.khorse.summon(horseIdentifier, player.getLocation());
 
         player.sendMessage(this.prefix + "Horse is summoned to your position");
       }
       else if (args[0].equalsIgnoreCase("getid"))
       {
         ...
         if (!this.khorse.isOnHorse(player))
         {
           player.sendMessage(this.prefix + ChatColor.GOLD + "You must be riding a horse to use this command");
           return true;
         }
         ...
         if (!this.khorse.isOwnedHorse(horse))
         {
           player.sendMessage(this.prefix + ChatColor.GOLD + "This horse is not protected");
           return true;
         }
         ...
         player.sendMessage(this.prefix + "Horse identifier: " + this.khorse.getHorseIdentifier(horse.getUniqueId()));
       }
       ...
       {
         sender.sendMessage(this.prefix + "HorseKeeper version " + this.version + " - Created by Falistos/BritaniaCraft (falistos@gmail.com)");
       }
       else if (args[0].equalsIgnoreCase("admin"))
       {
         ...
         if (args[1].equalsIgnoreCase("list"))
         {
           if (args.length < 3)
           {
             sender.sendMessage(this.prefix + ChatColor.GOLD + "Missing player name");
             return true;
           }
           ...
           List horsesList = this.khorse.getOwnedHorses(playerName);
         ...
         if ((e instanceof EntityDamageByEntityEvent))
         {
           ...
           if ((e1.getDamager() instanceof Player))
           {
             ...
             if (!perm.has(damager, "horsekeep.bypass.protection"))
             {
               ...
               {
                 damager.sendMessage(this.prefix + ChatColor.GOLD + "You can't attack an owned horse");
                 e1.setCancelled(true);
               }
               else if ((this.khorse.canMountHorse(damager, horse)) && (getConfig().getBoolean("disableHorseDamageFromMembers"))) {
                 damager.sendMessage(this.prefix + ChatColor.GOLD + "You can't attack this horse, if you are the owner or member of it");
                 e1.setCancelled(true);
               }
             }
           }
           else if ((e1.getDamager() instanceof Projectile))
           {
             ...
             if ((projectile.getShooter() instanceof Player))
             {
               ...
               if (!perm.has(shooter, "horsekeep.bypass.protection"))
               {
                 if (getConfig().getBoolean("disableHorseDamage"))
                 {
                   shooter.sendMessage(this.prefix + ChatColor.GOLD + "You can't attack an owned horse");
                   e.setCancelled(true);
                 }
                 else if ((this.khorse.canMountHorse(shooter, horse)) && (getConfig().getBoolean("disableHorseDamageFromMembers"))) {
                   shooter.sendMessage(this.prefix + ChatColor.GOLD + "You can't attack this horse, if you are the owner or member of it");
                   e.setCancelled(true);
                 }
               }
             }
   ...
   public void PotionsSplash(PotionSplashEvent e)
   {
     if ((e.getEntity().getShooter() instanceof Player)) {
       ...
       Collection AffectedEntities = e.getAffectedEntities();
       ...
       while (it.hasNext())
       {
         ...
         if (this.khorse.isHorse(entity))
         {
           if ((this.khorse.isOwnedHorse(entity.getUniqueId())) && (!perm.has(thrower, "horsekeep.bypass.protection")))
           {
             if (getConfig().getBoolean("disableHorseDamage"))
             {
               message = this.prefix + ChatColor.GOLD + "You can't attack an owned horse";
               cancelEvent = true;
             }
             else if ((this.khorse.canMountHorse(thrower, entity)) && (getConfig().getBoolean("disableHorseDamageFromMembers"))) {
               message = this.prefix + ChatColor.GOLD + "You can't attack this horse, if you are the owner or member of it";
               cancelEvent = true;
             }
           }
         }
   ...
   public void onDeath(EntityDeathEvent e) {
     ...
     if ((this.khorse.isHorse(entity)) && 
       (this.khorse.isOwnedHorse(entity.getUniqueId())))
     {
       String ownerName = this.khorse.getHorseOwner(entity);
       Player owner = Bukkit.getPlayerExact(ownerName);
 
       if (owner.isOnline()) owner.sendMessage(this.prefix + ChatColor.RED + "One of your horses is dead (" + this.khorse.getHorseIdentifier(entity.getUniqueId()) + ")");
     ...
     if (this.khorse.isHorse(event.getRightClicked()))
     {
       ...
       if (!this.khorse.isOwnedHorse(horse.getUniqueId()))
       {
         if (event.getPlayer().getItemInHand().getType() == Material.SADDLE)
         {
           ...
           List groups = getConfig().getMapList("groups");
           ...
           if (!perm.has(event.getPlayer(), "horsekeep.bypass.limit"))
           {
             for (Map item : groups)
             {
               for (Map.Entry group : item.entrySet())
               {
                 if (perm.has(event.getPlayer(), "horsekeep.groups." + group.getKey()))
                 {
                   limit = ((Integer)group.getValue()).intValue();
                 }
               }
             }
             ...
             {
               event.getPlayer().sendMessage(this.prefix + ChatColor.GOLD + "You have reached the maximum limit of horses you can protect - This horse will not be protected");
               return;
             }
           }
 
           if (event.getPlayer().getName().isEmpty()) { event.getPlayer().sendMessage(this.prefix + ChatColor.RED + "Error while setting up protection"); return;
           }
            this.khorse.setHorseOwner(event.getPlayer(), horse);
 
           horse.getWorld().playSound(horse.getLocation(), Sound.LEVEL_UP, 10.0F, 1.0F);
 
           event.getPlayer().sendMessage(this.prefix + "You protected this horse! Now use " + ChatColor.GOLD + "/horse id " + this.khorse.getHorseIdentifier(this.khorse.getHorseUUID(horse)) + ChatColor.AQUA + " <new-name>");
         }
 
       }
       else if ((!this.khorse.canMountHorse(event.getPlayer(), horse)) && (!perm.has(event.getPlayer(), "horsekeep.bypass.mount")))
       {
         event.getPlayer().sendMessage(this.prefix + ChatColor.RED + "This horse belongs to " + ChatColor.AQUA + this.khorse.getHorseOwner(horse));
 
         if (perm.has(event.getPlayer(), "horsekeep.admin")) event.getPlayer().sendMessage(this.prefix + "Horse Identifier: " + this.khorse.getHorseIdentifier(this.khorse.getHorseUUID(horse)));
   ...
   public void onAnimalTame(EntityTameEvent e)
   {
     ...
     if ((this.khorse.isHorse(tamedAnimal)) && (!this.khorse.isOwnedHorse(tamedAnimal)))
     {
       player.sendMessage(this.prefix + "You tamed a horse! Now right-click with a saddle on horse to protect it");
     }
   }
   ...
   public void onVehicleEnter(VehicleEnterEvent event)
   {
     ...
     if (this.khorse.isHorse(event.getVehicle()))
     {
       ...
       if (this.khorse.isOwnedHorse(horse.getUniqueId()))
       {
         ...
         {
           player.sendMessage(this.prefix + "This horse belongs to " + ChatColor.AQUA + this.khorse.getHorseOwner(horse));
